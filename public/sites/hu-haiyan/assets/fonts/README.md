...
- `display-serif.woff2`：Noto Serif SC，字重范围 400–500，181,512 bytes。
- `body-sans.woff2`：Noto Sans SC，字重范围 400–500，141,220 bytes。
- 合计 322,732 bytes（约 315 KiB）。
- 两个字体均为真实变量字体，默认字重 400；界面建议使用 400，强调使用 500。
- 字符范围仅包含当前网站 HTML/JS 字符、ASCII、常用中英文标点及中文数字变体。每个文件包含 609 个 Unicode 映射；已验证当前网站汉字没有缺字，源字体支持的请求字符均保留。
...
文件由项目内 `qa/hu-haiyan/fonts/build_fonts.py` 生成。为了减少静态作品集的首次下载量，不保留完整 GB2312 字库；每次修改网站文案后，应重新运行两个字体的生成命令。脚本自动收集当前 HTML/JS 字符并验证生成字库没有丢失源字体支持的字符。源 TTF 和转换依赖放在 QA 目录，不随网站部署。
